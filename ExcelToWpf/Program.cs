...
namespace ExcelToWpf
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string targetFile = @"C:\Users\hjd\Desktop\20180605検査員の作業範囲.xlsx";

            Excel.Application xlApp = new Excel.Application();

            var workbook = xlApp.Workbooks.Open(targetFile);
            ...
            foreach (Excel.Worksheet sheet in workbook.Worksheets)
            {
                ...
                if (sheet.Name == "print")
                {
                    ...
                    Console.WriteLine(String.Join(" ", res.FirstOrDefault().Value.Select(r => r.Position())));
                }
        ...
        private static void goLeft(int i, int j, Border[,] rawborder, List<Border> list)
        {
            ...
            if (rawborder[i, j].T > 0)
            {
                ...
                if (j - 1 >= 0 && rawborder[i, j - 1].T > 0)
                {
                    if (list.Contains(rawborder[i, j]))
                    {
                        //finish
                        return;
                    }
        ...
        private static void printBorderInfo(Excel.Range rg, int row, int col)
        {
            var border = rg.Borders;
            Console.Write("cell(" + row + " " + col + ") " + getborder(border[Excel.XlBordersIndex.xlEdgeLeft]));
            Console.Write(" " + getborder(border[Excel.XlBordersIndex.xlEdgeTop]));
            Console.Write(" " + getborder(border[Excel.XlBordersIndex.xlEdgeRight]));
            Console.WriteLine(" " + getborder(border[Excel.XlBordersIndex.xlEdgeBottom]));
        }
